...
# Run SQL Server
docker run --network $NETWORK --platform linux/amd64 -e "ACCEPT_EULA=Y" -e "SA_PASSWORD=$SQL_PASSWORD" -p 1433:1433 --name sql -h $SQL_SERVER -d mcr.microsoft.com/mssql/server:2022-latest
...
docker run --network $NETWORK --platform linux/amd64 -e SQLFQDN=$SQL_SERVER -e SQLUSER=sa -e SQLPASS=$SQL_PASSWORD -e SQLDB=mydrivingDB $CONTAINER_REGISTRY.azurecr.io/dataload:1.0
